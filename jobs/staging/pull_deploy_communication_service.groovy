pipelineJob('pull_deploy_communication_service') {
    description('Pull and Deploy Job for Communication Service with Environment Support')

    parameters {
        stringParam('ENVIRONMENT', 'staging', 'Environment name (e.g., staging, production)')
        stringParam('SERVICE_NAME', 'communication-service', 'Service name (e.g., communication-service, building-service)')
        stringParam('DB_NAME', 'communication_db', 'Database name associated with the service')
...
pipeline {
    ...
    environment {
        ...
        SEQUELIZE_DB = "\${env.STAGING_COMMUNICATION_SERVICE_SEQUELIZE_DB}"
        SEQUELIZE_USERNAME = "\${env.STAGING_COMMUNICATION_SERVICE_SEQUELIZE_USERNAME}"
        SEQUELIZE_PASSWORD = "\${env.STAGING_COMMUNICATION_SERVICE_SEQUELIZE_PASSWORD}"
        SEQUELIZE_HOST = "\${env.STAGING_COMMUNICATION_SERVICE_SEQUELIZE_HOST}"
        SEQUELIZE_PORT = "\${env.STAGING_COMMUNICATION_SERVICE_SEQUELIZE_PORT}"
        NODE_ENV = "development"
        SERVICE_PORT = "\${env.STAGING_COMMUNICATION_SERVICE_PORT}"
