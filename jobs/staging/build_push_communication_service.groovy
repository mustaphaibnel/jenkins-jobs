pipelineJob('build_push_communication_service') {
    description('Build and Push Job for communication-service with configurable parameters for AWS credentials, GitHub credentials, Docker tag, and AWS account.')
    parameters {
        stringParam('REPO_URL', 'https://github.com/viven-app/communication-service.git', 'GitHub Repository URL')
        stringParam('ORG_NAME', 'viven-app', 'Organization Name')
        stringParam('BRANCH', 'DEVOPS-1', 'Git Branch')
        stringParam('ECR_REPO', 'communication-service', 'AWS ECR Repository Name')
...
pipeline {
    ...
    parameters {
        string(name: 'REPO_URL', defaultValue: 'https://github.com/viven-app/communication-service.git', description: 'GitHub Repository URL')
        ...
        string(name: 'ECR_REPO', defaultValue: 'communication-service', description: 'AWS ECR Repository Name')
